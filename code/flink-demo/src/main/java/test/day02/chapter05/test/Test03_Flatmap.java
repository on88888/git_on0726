 ...
 * @author on
 * @Description TODO
 * @since 2021年12月10日 21:10:47
 */
//练习题：自己模拟数据，使用flatmap算子将WaterSensor类型的数据中vc值超过40的数据过滤出来并重新放回流中。
public class Test03_Flatmap {
    public static void main(String[] args) {
        ...
        waterSensorDS.flatMap(new FlatMapFunction<WaterSensorTest, WaterSensorTest>() {
            @Override
            public void flatMap(WaterSensorTest value, Collector<WaterSensorTest> out) throws Exception {
                if (value.getVc() > 40){
                    out.collect(value);
                }
            }
